...
#define kThumbnailLength    150.0f
...
- (void)didSelectAsset:(ALAsset *)asset
{
    if (_indexPathsForSelectedItems.count==1) {
        ZYQAssetPickerController *vc = (ZYQAssetPickerController *)self.navigationController;
        if ([vc.delegate respondsToSelector:@selector(assetPickerController:didDeselectAsset:)])
            [vc.delegate assetPickerController:vc didDeselectAsset:[_indexPathsForSelectedItems firstObject]];
        
        [_indexPathsForSelectedItems removeLastObject];
...
//        }
        [ProgressHUDUtils dismissProgressHUDErrorWithStatus:@"请选择一张照片!"];
    }else{
...
- (void)bind:(ALAssetsGroup *)assetsGroup
{
    ...
    self.imageView.image        = [UIImage imageWithCGImage:posterImage scale:1.0f orientation:UIImageOrientationUp];
