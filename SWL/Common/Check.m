...
//判断文本是否为空，YES:6-14位,NO:反之
+(bool)checkRange:(NSString*)strmessage
{
    bool isflag = YES;
    strmessage=[strmessage stringByReplacingOccurrencesOfString:@" " withString:@""];
    if(strmessage.length<6 || strmessage.length>14)
    {
        isflag=NO;
    }
...
// 邀请码是否6位,YES:6位,NO:反之
+(bool)checkCode:(NSString*)strmessage
{
    bool isflag = YES;
    strmessage=[strmessage stringByReplacingOccurrencesOfString:@" " withString:@""];
...
+ (bool)checkUserRange20:(NSString *)strmessage
{
    bool isflag = YES;
    strmessage=[strmessage stringByReplacingOccurrencesOfString:@" " withString:@""];
